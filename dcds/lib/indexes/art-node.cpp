...
void ARTNode_4::insertChild(key_unit_t &partial_key, ARTNode *child) {
  ...
  if (unlikely(_partial_keys[pos] == partial_key) && _children[pos] != nullptr) {
    ...
    if (pos != this->n_children) {
      ...
      std::memmove(_children.data() + pos + 1, _children.data() + pos, (this->n_children - pos) * sizeof(ARTNode *));
    }
...
ARTNode *ARTNode_4::shrink() { throw std::runtime_error("Node_4 can not shrink"); }
...
ARTNode *ARTNode_16::getChild(key_unit_t &partial_key) {
  ...
  int bitfield =
      _mm_movemask_epi8(_mm_cmpeq_epi8(_mm_set1_epi8(partial_key), _mm_loadu_si128((__m128i *)&_partial_keys[0]))) &
...
ARTNode *ARTNode_256::getChild(key_unit_t &partial_key) { return _children[partial_key]; }

void ARTNode_256::insertChild(key_unit_t &partial_key, ARTNode *child) { _children[partial_key] = child; }

ARTNode *ARTNode_256::grow() { throw std::runtime_error("Node 256 cannot grow"); }
