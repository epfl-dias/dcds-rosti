...
#ifndef DCDS_ART_NODE_HPP
#define DCDS_ART_NODE_HPP
...
class ARTNode {
 public:
  ...
  uint16_t prefix_len;                                   // length of compressed path
  uint8_t n_children;                                    // valid number of children
  std::vector<uint8_t> prefix;                           //  change deque to vector
  std::atomic<uint64_t> typeVersionLockObsolete{0b100};  // lock for synchronization

  explicit ARTNode(size_t prefix_len_) : prefix_len(prefix_len_), n_children(0) {}
  virtual ~ARTNode();
  ...
  bool isLocked(uint64_t version) const { return ((version & 0b10) == 0b10); }
...
class ARTInnerNode : public ARTNode {
 public:
  explicit ARTInnerNode(size_t prefix_len_) : ARTNode(prefix_len_) {}
...
class ARTLeaf : public ARTNode {
 public:
  ...
  static ARTLeaf<K, V> *create(ARTKey<K> key, V val, bool single_value) {
    if (single_value) {
      auto *memAlloc = malloc(sizeof(ARTLeafSingleValue<K, V>));
  ...
  static ARTLeaf<K, V> *create(const key_unit_t *key, V val, bool single_value) {
    if (single_value) {
      auto *memAlloc = malloc(sizeof(ARTLeafSingleValue<K, V>));
 ...
 protected:
  explicit ARTLeaf(const key_unit_t *key, V value, size_t const key_len) { _key = ARTKey<K>(key_len); }
...
class ARTLeafSingleValue : public ARTLeaf<K, V> {
 ...
 private:
  ...
  explicit ARTLeafSingleValue(ARTKey<K> key, V val) : ARTLeaf<K, V>(key), _value(val) {}
...
class ARTLeafMultiValue : public ARTLeaf<K, V> {
 public:
  ...
  void remove(V &val) override { _values.erase(std::remove(_values.begin(), _values.end(), val), _values.end()); }
 ...
 private:
  ...
  ARTLeafMultiValue(ARTKey<K> key, V val) : ARTLeaf<K, V>(key, val) { _values.emplace_back(val); }
...
class ARTNode_4 : public ARTInnerNode {
 public:
  ...
  static ARTNode_4 *create(size_t prefix_len) {
    static thread_local auto &artAllocator = ARTAllocator<sizeof(ARTNode_4)>::getInstance();
...
class ARTNode_16 : public ARTInnerNode {
 public:
  ...
  static ARTNode_16 *create(const ARTNode_4 &node4) {
    static thread_local auto &artAllocator = ARTAllocator<sizeof(ARTNode_16)>::getInstance();
    auto *mem = artAllocator.allocate();
    ...
    std::copy_n(node4._partial_keys.begin(), node4.n_children, node16->_partial_keys.begin());
    std::copy_n(node4._children.begin(), node4.n_children, node16->_children.begin());
    return node16;
  }
...
class ARTNode_48 : public ARTInnerNode {
 public:
  ...
  static ARTNode_48 *create(const ARTNode_16 &node16) {
    static thread_local auto &artAllocator = ARTAllocator<sizeof(ARTNode_48)>::getInstance();
    auto *mem = artAllocator.allocate();
    ...
    std::copy_n(node16._children.begin(), node16.n_children, node48->_children.begin());
...
class ARTNode_256 : public ARTInnerNode {
 public:
  explicit ARTNode_256(size_t prefix_len) : ARTInnerNode(prefix_len) { this->_children.fill(nullptr); }

  ~ARTNode_256() override = default;

  static ARTNode_256 *create(const ARTNode_48 &node48) {
    static thread_local auto &artAllocator = ARTAllocator<sizeof(ARTNode_256)>::getInstance();
    auto *mem = artAllocator.allocate();
